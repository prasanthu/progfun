...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def popular(a: Tweet, b: Tweet) = {
      if (a.retweets > b.retweets) a
      else b
    }
    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) popular(elem, right.mostRetweeted)
    else if (right.isEmpty) popular(elem, left.mostRetweeted)
    else popular(popular(elem, right.mostRetweeted), left.mostRetweeted)
  }

  def descendingByRetweet: TweetList = {
    val largest = this.mostRetweeted
    new Cons(largest, this.remove(largest).descendingByRetweet)
  }
